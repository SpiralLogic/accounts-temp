<?php
  ...
  class JsHttpRequest {
        ...
        if ($headersSent) {
          Event::error(
            "HTTP headers are already sent" . ($line !== null ? " in $file on line $line" :
              " somewhere in the script") . ". " . "Possibly you have an extra space (or a newline) before the first line of the script or any library. " . "Please note that JsHttpRequest uses its own Content-Type header and fails if " . "this header cannot be set. See header() function documentation for more details"
          );
          exit();
        }
      } else {
    ...
    function _correctSuperglobals() {
      ...
      $_REQUEST
                                 = (isset($_COOKIE) ? $_COOKIE : array()) + (isset($_POST) ? $_POST : array()) + (isset($_GET) ? $_GET : array());
    ...
    function _toUtf8_callback(&$v, $k, $fromEnc) {
      if ($v === null || is_bool($v) || is_object($v)) {
        return;
      }

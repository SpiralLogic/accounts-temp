<?php
   ...
   */
  if (!defined("K_PATH_FONTS")) {
    define ("K_PATH_FONTS", PATH_VENDOR . 'fonts/');
  }
  if (!defined("K_PATH_CACHE")) {
    define ("K_PATH_CACHE", PATH_VENDOR . 'fonts/');
  }
  ...
  class TCPDF {
    ...
    function AddPage($orientation = '', $format = '') {
      ...
      $this->_out(
        "" . $prev_linestyleWidth . " " . $prev_linestyleCap . " " . $prev_linestyleJoin . " " . $prev_linestyleDash . " " . $prev_DrawColor . " " . $prev_FillColor . ""
      );
    ...
    function Annotation($x, $y, $w, $h, $text, $opt = array('Subtype' => 'Text')) {
      $this->PageAnnots[$this->page][] = array(
        'x'   => $x,
        'y'   => $y,
        'w'   => $w,
        'h'   => $h,
    ...
    function _dolinethrough($x, $y, $txt) {
      ...
      return sprintf(
        '%.2f %.2f %.2f %.2f re f',
        $x * $this->k,
        ($this->h - ($y - ($this->FontSize / 2) - $up / 1000 * $this->FontSize)) * $this->k,
        $w * $this->k,
    ...
    function _addfield($type, $name, $x, $y, $w, $h, $prop) {
      ...
      $this->javascript .= sprintf(
        "f" . $name . "=this.addField('%s','%s',%d,[%.2f,%.2f,%.2f,%.2f]);",
        $name,
        $type,
        $this->PageNo() - 1,
        $x * $k,
      ...
      while ($key <= $maxel) {
        ...
        if (preg_match($tagpattern, $element)) {
          ...
          if ($element{0} == '/') {
            ...
            if (($dom[$key]['value']{0} == "h") AND (intval($dom[$key]['value']{1}) > 0) AND (intval(
              $dom[$key]
              ['value']{1}
            ) < 7)
            ) {
          ...
          if (isset($tag['attribute']['src'])) {
            ...
            if (($type == "eps") OR ($type == "ai")) {
              $this->ImageEps(
                ...
                $this->GetX(),
                $this->GetY(),
                $this->pixelsToUnits($tag['attribute']['width']),
                $this->pixelsToUnits($tag['attribute']['height']),
                '',
                true,
                $align
              );

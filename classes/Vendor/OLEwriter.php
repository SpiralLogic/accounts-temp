...
*/
	...
	class Spreadsheet_Excel_Writer_Format
	{
		...
		function getXf($style)
		{
			...
			$atr_bdr = ($this->_bottom || $this->_top || $this->_left || $this->_right) ? 1 : 0;
			$atr_pat = (($this->_fg_color != 0x40) || ($this->_bg_color != 0x41) || $this->_pattern) ? 1 : 0;
		...
		function getFont()
		{
			...
			if ($this->_BIFF_version == 0x0500) {
				...
				$data = pack("vvvvvCCCCCC", $dyHeight, $grbit, $icv, $bls, $sss, $uls, $bFamily, $bCharSet, $reserved, $cch, $encoding);
			}
	...
	class Spreadsheet_Excel_Writer_Parser
	{
		...
		function _fact()
		{
			...
			die("Syntax error: " . $this->_current_token . ", lookahead: " . $this->_lookahead . ", current char: " . $this->_current_char);
		}
	...
	class Spreadsheet_Excel_Writer_Worksheet extends Spreadsheet_Excel_Writer_BIFFwriter
	{
        ...
        ***********************************************/
		...
		function mergeCells($first_row, $first_col, $last_row, $last_col)
		{
			...
			$header = pack("vv", $record, $length);
			$data = pack("vvvvv", $cref, $first_row, $last_row, $first_col, $last_col);
			$this->_append($header . $data);
		}
		...
		function _positionImage($col_start, $row_start, $x1, $y1, $width, $height)
		{
			...
			$this->_storeObjPicture($col_start, $x1, $row_start, $y1, $col_end, $x2, $row_end, $y2);
		}
		...
		function _storeDataValidity()
		{
			...
			$header = pack('vv', $record, $length);
			$data = pack('vVVVV', $grbit, $horPos, $verPos, $objId, count($this->_dv));
			$this->_append($header . $data);

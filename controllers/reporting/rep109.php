  ...
  function print_sales_orders() {
    ...
    if ($email == 0) {
      if ($print_as_quote == 0) {
        ...
        $rep = new $report_type(_("ORDER"), "SalesOrderBulk", $security, User::_page_size());
      } elseif ($print_as_quote == 2) {
        /** @var \ADV\App\Reports\PDF|\ADV\App\Reports\Excel $rep */
        $rep = new $report_type(_("PROFORMA INVOICE"), "QuoteBulk", $security, User::_page_size());
      } elseif ($print_as_quote == 3) {
        /** @var \ADV\App\Reports\PDF|\ADV\App\Reports\Excel $rep */
        $rep = new $report_type(_("PROFORMA INVOICE"), "QuoteBulk", $security, User::_page_size());
      } else {
        /** @var \ADV\App\Reports\PDF|\ADV\App\Reports\Excel $rep */
        $rep = new $report_type(_("QUOTE"), "QuoteBulk", $security, User::_page_size());
      }
      $rep->currency = $cur;
      $rep->Font();
    ...
    for ($i = $from; $i <= $to; $i++) {
      ...
      if ($email == 1) {
        $rep           = new $report_type("", "", $security, User::_page_size());
        $rep->currency = $cur;
        $rep->Font();
        if ($print_as_quote == 1) {
          $rep->title    = _('QUOTE');
